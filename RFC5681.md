...
为了防止突发性到来的大量数据拥塞整个网络，如果 TCP发送端在开始向网络发送信息时没有掌握任何关于网络容量的信息，我们就**要求**(requires) 它缓慢地探测(slowly probe) 网络以判断网络的可用容量。在传输刚刚启动或者重传计时器所检测到的丢失已被修复时，我们使用慢启动算法以实现这种“ 缓慢地探测”。慢启动算法还可用于启动 TCP 发送端所使用的“ACK 时钟”(the ACK clock)，依靠该时钟， TCP 发送端可以使用“慢启动”、“拥塞控制”或者“丢失恢复”三种算法将数据发送到网络环境中。
