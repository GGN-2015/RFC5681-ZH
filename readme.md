...
上述 TCP 拥塞控制控制算法的一个已知的潜在问题是，它可能在有单长时间的闲置后突然向网路发送过多的数据。闲置了一段时间后，由于先前的所有 ACK 都已经离开了网络，TCP 发送端无法使用 ACK 时钟去控制新的段进入网络的速率。因此正如上文所述，TCP 有可能会在一段时间的闲置后以线性的速率向网络中发送许多尺寸为 $cwnd$ 的消息段从而拥塞网络。此外，不断变化的网络条件也可能使得先前记录在 $cwnd$ 中的“网络容量”已经不再符合当前的网络环境。
