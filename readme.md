...
快速重传和快速回复算法的实现相互关联，具体实现如下文所述：

1. 当第一个和第二个重复的 ACK 到来时，只要接收端发布的接受窗允许且“在传数据量” FlightSize 仍不超过 $cwnd+2\times SMSS$ 且新的数据段已经准备就绪，TCP 发送端就**应该**(SHOULD) 根据 [RFC3042] 的规定发送一个先前未被发送的数据段。具体而言，TCP 发送端**禁止**(MUST NOT) 根据这两个段改变 $cwnd$ 的取值 [RFC3042]。需要注意的是，对于那些使用了 SACK 技术 [RFC2018] 的发送端而言，它们**禁止**(MUST NOT) 发送新的数据，除非刚刚收到的重复确认胸袭包含了新的 SACK 信息。
